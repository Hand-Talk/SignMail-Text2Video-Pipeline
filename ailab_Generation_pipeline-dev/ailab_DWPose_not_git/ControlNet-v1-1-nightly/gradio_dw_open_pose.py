...
def process(det, pose, input_image, prompt, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta):
    ...
    with torch.no_grad():
        ...
        if det == 'None':
            ...
            if 'DW' in pose:
                detected_map, kp_vectors = dwprocessor(resize_image(input_image, detect_resolution))
            else:
...
with block:
    with gr.Row():
        gr.Markdown("## Control Stable Diffusion with DWPose or OpenPose")
    with gr.Row():
        with gr.Column():
            ...
            pose = gr.Radio(choices=["Openpose", "DWpose"], type="value", value="DWpose", label="processor")

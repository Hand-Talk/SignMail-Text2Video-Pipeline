...
def save_videos_grid(videos: torch.Tensor, path: str, rescale=False, n_rows=6, fps=8, save_frame=False):
    ...
    # new_shape = list(videos.shape)
    # new_shape = [1 if dim == 0 else dim for dim in new_shape]

    # Reshape only if there was a zero dimension
    if 0 in videos.shape:
        return 
        videos = videos.view(new_shape)

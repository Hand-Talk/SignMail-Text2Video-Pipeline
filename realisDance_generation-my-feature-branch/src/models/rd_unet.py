...
class RealisDanceUnet(nn.Module):
    ...
    def forward(
        ...
        # # Probabilistic conditioning mode selection
        # if text_embeddings is not None:
        #     rand_val = random.random()
        #     text_embeddings_expanded = text_embeddings.unsqueeze(1)
            
        #     if rand_val < 0.3:
        #         # 20% probability: Use only text embeddings
        #         encoder_hidden_states = text_embeddings_expanded
        #     elif rand_val < 0.5:
        #         # 30% probability: Use combination of text and encoder_hidden_states
        #         encoder_hidden_states = torch.cat([encoder_hidden_states, text_embeddings_expanded], dim=1)
        #     # else: 50% probability: Use only encoder_hidden_states (no change needed)
